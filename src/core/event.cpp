...
Event* Event::createNewFruitTreeEvent(Project *project)
{
    Event *event = new Event;
    event->put("event_group_type", "bg_event_group");
    event->put("event_type", EventType::FruitTree);
    event->put("fruit_tree_id", project->fruitTreeIds->first());
    return event;
}
...
OrderedJson::object Event::buildObjectCloneEventJSON(QMap<QString, QString> *mapNamesToMapConstants)
{
    OrderedJson::object eventObj;
    eventObj["type"] = "clone";
    eventObj["x"] = this->getS16("x");
...
OrderedJson::object Event::buildFruitTreeEventJSON()
{
    OrderedJson::object fruitTreeObj;
